...
describe Url do
  ...
  describe 'when access to the Google' do
    it 'should get title' do
      response = @url.response(FakeMessage.new('http://www.google.com'))
  ...
  describe 'when access to the Twitter' do
    it 'should get tweet' do
      response = @url.response(FakeMessage.new('https://twitter.com/#!/gongoZ/statuses/288559849971781633'))
  ...
  describe 'when recieved message' do
    it 'should not be accessed' do
      response = @url.response(FakeMessage.new('Tomochan: test'))
